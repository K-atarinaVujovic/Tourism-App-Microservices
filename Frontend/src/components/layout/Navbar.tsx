...
import { BookOpen, Grape, LayoutDashboard, PenLine, PersonStanding, Shield, Wheat } from "lucide-react";
import {
  NavigationMenu,
  NavigationMenuItem,
  NavigationMenuLink,
...
export default function Navbar() {
  ...
  const navItems = [
    { path: "/home", label: "Home", icon: LayoutDashboard, requiresAuth: null },
    { path: "/admin/users", label: "Users", icon: PersonStanding, requiresAuth: true, adminOnly: true },
    { path: "/login", label: "Log in", icon: Shield, requiresAuth: false },
    { path: `/profile/${user?.id}`, label: "My Profile", icon: Wheat, requiresAuth: true },
    { path: "/blogs", label: "Blogs", icon: BookOpen, requiresAuth: null },
    { path: "/blogs/create", label: "Create Blog", icon: PenLine, requiresAuth: true },
  ];

  const visibleItems = navItems.filter(({ requiresAuth, adminOnly }) => {
